...
public class JVM extends JDKApplication {
	...
	// "-classpath",
	// classpath,
	// EJE.options.getProperty("eje.java.assertions")
	// .equals("true") ? "-ea" : "-da",
	...
	// } catch (Exception exc) {
	// this.addOutputLine(BugReporter.getMessage(exc.toString(), "JVM",
	// "execute()"), errorStreamGetter);
	// }
	// }
	...
	// } catch (Exception exc) {
	// this.addOutputLine(BugReporter.getMessage(exc.toString(), "JVM",
	// "execute(args)"), errorStreamGetter);
	// }
	// }
	//
	// private void execute(File javaFile, File classFile, String[] commandLine)
	// throws IOException {
	// for (int i = 0; i < commandLine.length; ++i)
	// System.out.print(commandLine[i] + " ");
	// setFile(classFile);
	// Process execution = Runtime.getRuntime().exec(commandLine, null,
	// javaFile.getParentFile());
	// processes.addElement(execution);
	// outputStreamGetter = new OutputStreamGetter(execution.getInputStream(),
	// this);
	// errorStreamGetter = new ErrorStreamGetter(execution.getErrorStream(),
	// this);
	// outputStreamGetter.start();
	// errorStreamGetter.start();
	// EJEStatusBar.getStatusBar().changeColor(
	// EJE.resources.getString("build.execute.status"));
	// }
	//
	...
	public void processInput(String input) {
		if (isProcessing())
			try {
				((Process) processes.lastElement()).getOutputStream().write(
						input.getBytes());
